...
-- Combined schema for self-hosted PostgreSQL
-- Run: psql -U pricewise -d pricewise -f init.sql
-- ============================================
...
CREATE INDEX IF NOT EXISTS idx_locations_major ON locations(is_major) WHERE is_major = TRUE;
...
CREATE TABLE IF NOT EXISTS products (
  ...
  condition TEXT CHECK (condition IN ('brand_new', 'used', 'foreign_used', 'local_used', 'refurbished')),
  source_url TEXT,
  source TEXT CHECK (source IN ('jumia', 'konga', 'jiji', 'manual')),
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE INDEX IF NOT EXISTS idx_price_points_location ON price_points(location_id);
CREATE INDEX IF NOT EXISTS idx_price_points_recorded ON price_points(recorded_at DESC);
CREATE INDEX IF NOT EXISTS idx_price_points_product_location ON price_points(product_id, location_id, recorded_at DESC);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_latest_prices_pk ON latest_prices(id);
CREATE INDEX IF NOT EXISTS idx_latest_prices_product ON latest_prices(product_id);
CREATE INDEX IF NOT EXISTS idx_latest_prices_category ON latest_prices(category_id);

-- ============================================
-- MATERIALIZED VIEW: Product price summaries
-- ============================================
CREATE MATERIALIZED VIEW IF NOT EXISTS product_price_summaries AS
SELECT
  p.id AS product_id,
  p.name,
  p.slug,
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_product_summaries_pk ON product_price_summaries(product_id);
CREATE INDEX IF NOT EXISTS idx_product_summaries_category ON product_price_summaries(category_id);
...
BEGIN
  RETURN QUERY
  SELECT
    p.id, p.name, p.slug, p.category_id, p.subcategory, p.unit,
...
BEGIN
  ...
  WITH current_prices AS (
    SELECT pp.product_id, AVG(pp.price) AS avg_price
    FROM price_points pp
